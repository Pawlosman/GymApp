...
export default function Statistics({ user, profile }) {
  ...
  function calculateStats() {
    ...
    workouts.forEach(workout => {
      ...
      if (!exerciseStats[exerciseName]) {
        exerciseStats[exerciseName] = { totalWeight: 0, totalReps: 0, totalSets: 0 }
      }
  ...
  const sortedExercises = Object.entries(exerciseStats).sort(([, a], [, b]) => b.totalWeight - a.totalWeight)
        ...
        <>
          <div className="card mb-4 bg-primary text-white">
            <div className="card-body text-center">
              ...
              <p className="mb-0">
                {selectedMonth === 'all' ? `In ${selectedYear}` : `In ${MONTH_NAMES[selectedMonth]} ${selectedYear}`}
              </p>
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="card-body p-0">
              {sortedExercises.length === 0 ? (
                <div className="text-center py-5 text-muted">No workout data available for this period</div>
              ) : (
                  ...
                  <table className="table table-striped table-hover mb-0">
                    ...
                    <tbody>
                      {sortedExercises.map(([name, stats]) => (
                        <tr key={name}>
                          ...
                          <td className="text-end">{totalWeight > 0 ? ((stats.totalWeight / totalWeight) * 100).toFixed(1) : 0}%</td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot className="table-secondary">
                      <tr>
                        ...
                        <td className="text-end"><strong>{Object.values(exerciseStats).reduce((sum, s) => sum + s.totalSets, 0)}</strong></td>
                        <td className="text-end"><strong>{Object.values(exerciseStats).reduce((sum, s) => sum + s.totalReps, 0)}</strong></td>
                        <td className="text-end"><strong>{totalWeight.toLocaleString()}</strong></td>
